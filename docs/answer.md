...
seqDiagram
    ...
    Note over DB: 1. FROM book b<br>(도서 테이블 데이터 소스 지정)
    Note over DB: 2. INNER JOIN category c ON b.category_id = c.id<br>(카테고리 매칭 및 가로 테이블 결합)
    Note over DB: 3. GROUP BY c.name<br>(동일 카테고리 이름별 세로 그룹 병합)
    Note over DB: 4. SELECT c.name, SUM(b.price), AVG(b.price)<br>(그룹별 총합/평균 집계 연산 및 컬럼 선별)
```
